...
class CerebellarNetwork(bp.DynSysGroup):
    ...
    def __init__(self, num_pf_bundles=5, num_pc=100, num_cn=40, num_io=40, **kwargs):
        ...
        Keyword Args:
            PF_I_OU0 (float): Baseline current for PF OU process (nA). Default: 1.3.
            ...
            IO_VT (float): Threshold potention for IO (mV). Default: -30.0.
            ...
            IO_g_gj (float): Gap junction conductance for IO network (mS/cm2). Default: 0.05. Higher conductance = higher coupling strength
        ...
        (stimPF_pre, stimPF_post) = (bm.arange(0, num_pf_bundles), bm.arange(0, num_pf_bundles))
        stimPF_conn = bp.conn.IJConn(stimPF_pre, stimPF_post)
        (stimIO_pre, stimIO_post) = (bm.arange(0, num_io), bm.arange(0, num_io))
        stimIO_conn = bp.conn.IJConn(stimIO_pre, stimIO_post)

        self.stim_to_pf = stimToPF(pre = self.stim, post = self.pf, conn =  stimPF_conn)
        self.stim_to_io = stimToIO(pre = self.stim, post = self.io.neurons, conn = stimIO_conn)
